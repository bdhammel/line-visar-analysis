...
def image_from_selector_params(data, params):
    image = core.ImageData(
                data.data[
                    params["str_ij"][1]:params["end_ij"][1], 
                    params["str_ij"][0]:params["end_ij"][0]
                    ])
    image.set_dimensions(
            xmin=params["str_xy"][0], xmax=params["end_xy"][0], 
            ymin=params["str_xy"][1], ymax=params["end_xy"][1]) 
    return image
...
def automated_anaysis(params, raw_data=None):
    if raw_data is None:
        ...
        raw_data = core.load_image(
                params["file"], 
                xmin=streak_dims["xmin"], xmax=streak_dims["xmax"], 
                ymin=streak_dims["ymin"], ymax=streak_dims["ymax"],
                show=False)
