...
class ImageData:
    ...
    def show(self, title=None, color='gray', frame=True, figsize=(6,4)):
        """Display the image
...
def load_image(path, xmin, xmax, ymin, ymax):
    """Load the Raw Data
...
def filter_spectrogram(spec, ref_spec=None, start_frequency=None, end_frequency=None):
    ...
    print(str_frequency, end_frequency)
...
if __name__ == "__main__":
    ...
    with open("streak_camera_conf.json") as f:
        j = json.load(f)
        streak_cam = StreakCamera(j["sweep_speed"], j["slit_width"])
        del j
    ...
    data_selector, refrence_selector = select_working_data(raw_data)
    ...
    filtered_spec, str_freq, end_freq = filter_spectrogram(spec, ref_spec)
    filtered_spec.show('Filtered Spectrogram')


    # Invert the FFT to create the filtered image
    filtered_img, filtered_iimg = do_inv_fft(filtered_spec, cropped_data)
