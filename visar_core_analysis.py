...
class ImageData:
    ...
    data : 2D numpy array
        image data
    xx : 2D ndarray
        x values for each pixel in image data
    yy : 2D ndarray
        y values for each pixel in image data
    """
    ...
    def set_dimensions(self, xmin, xmax, ymin, ymax):
        ...
        -----------
        xmax : int
            the length of the sweep window
        ymax : int
            the size of the slit on the streak camera as it relates to
    ...
    def show(self, title=None, color='gray', frame=True,
             figsize=(6, 4), colorbar=False):
        """Display the image
        ...
        implot = ax.imshow(
            self._data, aspect='auto', extent=extent, origin="lower"
        )
...
class SpectrogramData(ImageData):
    ...
    def apply_fft(self, img):
        ...
        -----------
        img : ImageData
            object containing the data to Fourier transform
        """
    ...
    def show(self, title, color='hot', figsize=(6, 4), frame=True):
        ...
        implot = ax.imshow(
            self.pdata, aspect='auto', extent=extent, origin="lower"
        )
        implot.set_cmap(color)
        plt.ylim(-0.1, 0.1)
...
class IndexSelector:
    ...
    ----------
    _ax : Axis
        axis the plot is drawn on
    xdata : nparray
        xvalue of the line on the graph
    _cursor_index : int
        the index value the cursor was placed at
    _line
        object drawn on plot after user click
    _color
        color the line should be
    ...
    def __init__(self, line, color='g'):
        ...
        self.cidclick = self._ax.figure.canvas.mpl_connect(
            "pick_event", self._on_pick
        )
        self.cidkey = self._ax.figure.canvas.mpl_connect(
...
class DataSelector2D:
    ...
    ----------
    _ax : pyplot axis
        Axis the image is being displayed on
    _draw : boolean
        Should the selector track mouse movement to draw the
        rectangle in real time
    _rec : pyplot Rectangle
        Artist object drawn on figure
    _color : str
        color of rectangle g, r, b, etc...
    str_xy : tuple
        starting coordinates of rectangle (upper left)
    end_xy : tuple
        ending coordinates of rectangle (bottom right)
    width : int
        width of rectangle
    height : int
        height of rectangle
    ...
    def __init__(self, img, ax=None, color='r'):
        ...
        self.cidpress = self._ax.figure.canvas.mpl_connect(
            "button_press_event", self._mouse_press
        )
        self.cidrelease = self._ax.figure.canvas.mpl_connect(
            "button_release_event", self._mouse_release
        )
        self.cidmove = self._ax.figure.canvas.mpl_connect(
    ...
    def is_valid(self):
        ...
        -------
        bool
            True if the height and width are positive, False otherwise
        """
    ...
    def data(self):
        ...
        working_image = ImageData(
            self.img.data[self.str_ij[1]:self.end_ij[1],
...
class LockedSelector(DataSelector2D):
    ...
    def width(self):
        ...
        -------
        float
            if width is specified during initialization, return this with,
    ...
    def height(self):
        ...
        -------
        float
            if height is specified during initialization, return this with,
    ...
    def force_return(self):
        ...
        """
        self.str_xy = self.lock_start
        self.end_xy = self.str_xy + np.array([
            self.lock_width, self.lock_height
        ])
...
def prompt_for_path(path=None):
    ...
    ----------
    path : str, optional
        The path can be submitted via the function call,
...
def select_working_data(img):
    ...
    refrence_selector = LockedSelector(
        img, lock_start=data_selector.str_xy, lock_height=data_selector.height
    )
...
def show_averaged_fft(ref_spec):
    """Display a section of a spectrogram, averaged across each spectral
    dimension

    Parameters
    -----------
    ref_spec : SpectrogramData
        Reference spectrogram of constant fringes

    Returns
    -------
    matplotlib line
        line object of the averaged fft, with a "picker" value set
    """
...
def filter_spectrogram(spec, ref_spec=None,
                       str_frequency=None, end_frequency=None):
    """Remove unwanted frequencies by setting values in the spectrogram to 0

    Parameters
    -----------
    spec : SpectrogramData
        Spectrogram of the selected working data
    ref_spec : SpectrogramData
        Reference spectrogram of constant fringes
    start_frequency : float
        the starting frequency to keep (passed during analysis of background)
    end_frequency : float
        the last frequency to keep (passed during analysis of background)
    """
    ...
    filter_indices = np.where(
        (spec.k < str_frequency) | (spec.k > end_frequency)
    )
    filtered_spec[filter_indices, :] = 0

    filtered_spec = SpectrogramData(filtered_spec, wavenumbers=spec.k, x=spec.x)
...
def do_inv_fft(spec, cropped_data):
    ...
    cropped_data : ImageData
        selection of the data chosen by the user

    Returns
    -------
    filtered_img : ImageData
        filter image from real numbers
    filtered_iimg : ImageData
        filter image from imaginary numbers
    """
...
def vpf_from_etalon(etalons):
    ...
    for i, etalon in enumerate(etalons):
        print(
            "\t[{}]{thickness:_>20}{vpf:_>20}".format(
                i,
...
def vpf_thru_unshocked_window(vpf):
    ...
    for i, window in enumerate(windows):
        print("\t[{}]{material:_>20}".format(i, material=window["material"]))
    print("\t[{}]{:_>20}".format(i + 1, "custom"))
...
def get_velocity_map(img, ref, vpf):
    ...
    ref
    vpf : float
        velocity per fringe shift - dependent on the etalon chosen
    ...
    rel_phase = wrapped_phase - np.mean(wrapped_phase[..., :avg_to],
                                        axis=1,
                                        keepdims=True)
...
def get_velocity(velocity_map, ax, lineout_height=5.):
    ...
    Parameters
    ----------
    velocity_map : ImageData
        2D phase map of velocity
    lineout_height : negative int
        height of lineout used to select average of velocity.
    """
    lineout_selector = LockedSelector(
        velocity_map, ax=ax, lock_height=lineout_height
    )
...
def plot_velocity(time, velocity, picker=False):
    ...
    velocity ndarray float
        velocity values
    picker : bool
        enable picker so that a selector can be used on the plot
    """
